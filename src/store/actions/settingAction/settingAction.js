...
const updTagAction = (admin_ref, tag_name, tagData) => {
  ...
  return (dispatch) => {
    ...
    let obj = { admin_ref, tag_name };
    axios
      .post(`${baseUrl}tag/${tagData.id}`, obj)
      .then((success) => {
        console.log(success, "success");
        if (success.data.code) {
          toast.error(success.data.reason || success.data.message);
...
const assignAdAction = (productId, assignUserId, adminId, adIndex, history) => {
  return (dispatch) => {
    ...
    axios
      .post(`${baseUrl}adassigntoanotheruser`, obj)
      .then((success) => {
        console.log(success, "success");
        if (success.data.code) {
          toast.error(success.data.reason || success.data.message);
